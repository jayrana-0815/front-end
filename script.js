...
if (screenName === "farmer" && !farmerPlanLoaded) {
    loadFarmerPlan();
}
...
menu.innerHTML = `
    ...
    <div class="menu-item" data-screen="farmer">
...
function renderFarmerPlan(result) {
    ...
    if (taskList) {
        taskList.innerHTML = tasks.length
            ...
            : "<li>No tasks recommended for today.</li>";
    }
...
async function loadFarmerPlan() {
    ...
    const stage = document.getElementById("farmerGrowthStage");
    const moisture = document.getElementById("farmerSoilMoisture");
    if (!crop || !stage || !moisture) return;
    setFarmerStatus("Loading your farm plan…");
    ...
    try {
        const response = await fetch(`${API_BASE}/farmer/plan?${params.toString()}`);
        if (!response.ok) {
            const error = await response.json().catch(() => ({}));
            throw new Error(error.detail || `Farm plan request failed (${response.status})`);
        }
        ...
        if (!hasData) {
            setFarmerStatus("No farm recommendations are available for these selections.", "empty");
            return;
        }
        ...
        console.error("Farmer plan request failed", error);
        setFarmerStatus(`Unable to load farm plan: ${error.message}`, "error");
    } finally {
...
function extractLocation(message) {
    const match = message.match(
        /\bin\s+([a-zA-Z][a-zA-Z ]*?)(?:\s+(?:right now|rn|today|tomorrow)|[?.!,]|$)/i
    );
...
if (requestedLocation) {
    try {
        const locationResponse = await fetch(
            `${API_BASE}/forecast?location=${encodeURIComponent(requestedLocation)}`
        );
        if (locationResponse.ok) {
            const locationForecast = await locationResponse.json();
            applyForecast(locationForecast, {
                ...(locationForecast.location || {}),
                name: requestedLocation
            });
        }
    } catch (error) {
        console.error("Requested location lookup failed", error);
    }
}
...
try {
    ...
    const result = await response.json();
    latestForecast = result.forecast || latestForecast;
    return result.ai_response || "The assistant returned no response.";
} catch (error) {
    console.error("Weather assistant request failed", error);
    return `I could not reach the weather assistant. ${error.message}`;
}
...
function startVoiceInput() {
    ...
    recognition.onstart = () => showNotification("🎙️ Listening...");
    recognition.onerror = event => {
        console.error("Voice input failed", event.error);
        showNotification(`Voice input failed: ${event.error}`);
    };
...
function () {
    ...
    updateWeatherUI();
    initializeFarmerMode();

    console.log(
        "🌦️ WeatherGPT loaded successfully"
    );

    async function initializeFarmerMode() {
        if ("geolocation" in navigator) {
            navigator.geolocation.getCurrentPosition(
                position => loadWeatherForCoordinates(
                    position.coords.latitude,
                    position.coords.longitude
                ),
                error => {
                    console.info("Using default farmer location", error.message);
                    loadWeatherForCity(farmerLocation.name);
                },
                { enableHighAccuracy: false, timeout: 8000, maximumAge: 900000 }
            );
        } else {
            await loadWeatherForCity(farmerLocation.name);
        }

        refreshAlerts();
        window.setInterval(refreshAlerts, 15 * 60 * 1000);
    }

    async function loadWeatherForCity(city) {
        try {
            const response = await fetch(
                `${API_BASE}/forecast?location=${encodeURIComponent(city)}`
            );
            if (!response.ok) {
                const error = await response.json().catch(() => ({}));
            ...
            showScreen("home");
            showNotification(`Weather updated for ${farmerLocation.name}`);
        } catch (error) {
            console.error("Location weather request failed", error);
            showNotification(`Unable to load weather: ${error.message}`);
        }
    }
    ...
    async function loadWeatherForCoordinates(latitude, longitude) {
        try {
            const response = await fetch(
                `${API_BASE}/weather/coordinates?lat=${latitude}&lon=${longitude}`
            );
            if (!response.ok) {
                const error = await response.json().catch(() => ({}));
                throw new Error(error.detail || `Weather request failed (${response.status})`);
            ...
            const result = await response.json();
            applyForecast(result, result.location || {
                name: "Current location",
                country: ""
            });
            showNotification(`Weather updated for ${farmerLocation.name}`);
        } catch (error) {
            console.error("Coordinate weather request failed", error);
            showNotification(`Unable to load live weather: ${error.message}`);
        }
    }

    function applyForecast(result, location) {
        const current = result.current || result.forecast?.current || {};
        const today = result.today ||
            result.forecast?.today ||
            result.daily?.[0] ||
            result.forecast?.daily?.[0] ||
            {};
        latestForecast = result.forecast || result;
        farmerLocation = {
            ...farmerLocation,
            ...location,
            latitude: result.latitude ?? location.latitude ?? farmerLocation.latitude,
            longitude: result.longitude ?? location.longitude ?? farmerLocation.longitude
        };
        const farmerLocationLabel = document.getElementById("farmerLocationLabel");
        if (farmerLocationLabel) {
            farmerLocationLabel.textContent = `${farmerLocation.name || "Current location"}${farmerLocation.country ? `, ${farmerLocation.country}` : ""}`;
        }
        weatherData = {
            city: farmerLocation.name || farmerLocation.city || "Current location",
            country: farmerLocation.country || "India",
            temperature: Math.round(Number(current.temp ?? current.temperature ?? 0)),
            feelsLike: Math.round(Number(current.feels_like ?? current.apparent_temperature ?? 0)),
            condition: weatherCodeToCondition(current.weather_code),
            humidity: Math.round(Number(current.humidity ?? current.relative_humidity ?? 0)),
            wind: Math.round(Number(current.wind_speed ?? current.wind ?? 0)),
            rain: Math.round(Number(today.precip_prob ?? today.precipitation_probability ?? 0)),
    ...
    function weatherCodeToCondition(code) {
        ...
        if ([0, 1].includes(numericCode)) return "Sunny";
        if ([2, 3, 45, 48].includes(numericCode)) return "Cloudy";
        if ([51, 53, 55, 56, 57, 61, 63, 65, 66, 67, 80, 81, 82].includes(numericCode)) return "Rainy";
        if ([71, 73, 75, 77, 85, 86].includes(numericCode)) return "Snowy";
    ...
    async function refreshAlerts() {
        try {
            const response = await fetch(
                `${API_BASE}/alerts?lat=${farmerLocation.latitude}&lon=${farmerLocation.longitude}`
            );
            if (!response.ok) {
                const error = await response.json().catch(() => ({}));
                throw new Error(error.detail || `Alert request failed (${response.status})`);
            }
            const result = await response.json();
            const alerts = result.alerts || [];
            const summary = document.querySelector("#alertsScreen .alert-summary h2");
            if (summary) summary.textContent = `${alerts.length} Active Alerts`;
            if (alerts.length && notificationButton) {
                notificationButton.setAttribute("aria-label", `${alerts.length} active weather alerts`);
            }
        } catch (error) {
